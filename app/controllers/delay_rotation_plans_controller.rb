...
class DelayRotationPlansController < ApplicationController
  ...
  before_action :can_be_delayed_validation, only: [:new]

  def index
    @procedures = @user.procedures.where(code: 'T-PPR203').sort_by &:created_at
  end
  ...
  private
    ...
    def can_be_delayed_validation
      @procedure = Procedure.find(params[:parent_id])
      
      sub_procedures = @procedure.sub_procedures.where(code: "T-PPR203", state: ["in_draft","in_progress"]) #Busca los tramites de prorroga hijos
      ...
      unless concrete_procedure.can_be_delayed?
        redirect_to rotation_plan_path(@procedure)
        flash[:error] = 'La solicitud no puede ser prorrogada.'
      end
